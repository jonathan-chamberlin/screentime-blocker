    ...
    async function loadRealData() {
      const result = await fetch('/api/usage').then(r => r.json());
      return {
    ...
    function generateTestData() {
      ...
      for (let i = 180; i >= 0; i--) {
        ...
        for (let s = 0; s < sessionsToday; s++) {
          const productiveMinutes = Math.floor(rand() * 90) + 10;
    ...
    function renderHeatmap(summaries, { title, colorPrefix, valueKey, unitLabel }) {
      ...
      for (let i = 0; i <= 4; i++) {
        legend.innerHTML += `<div class="heatmap-legend-cell ${i > 0 ? `${colorPrefix}-${i}` : ''}" style="${i === 0 ? 'background:var(--surface-hover)' : ''}"></div>`;
      }
    ...
    function renderTopBlockedSites(history) {
      ...
      if (sorted.length === 0) {
        section.innerHTML += `<p style="color: var(--text-muted); font-size: 14px;">No blocked site data recorded yet.</p>`;
        return section;
      }
    ...
    function renderAvgSession(metrics) {
      ...
      section.innerHTML = `
        ...
        <div class="stat-value" style="font-size: 36px; margin-top: 8px;">${metrics.avgSessionMinutes} <span style="font-size: 18px; color: var(--text-muted);">min</span></div>`;
      return section;
    }
